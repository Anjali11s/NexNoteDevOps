        ...
        switch (action.type) {
          case 'UPDATE_NOTE':
            ...
            try {
              ...
              const currentNote = await api.get(`/notes/${realId}`);

              // Compare timestamps to detect conflicts
              const serverUpdatedAt = new Date(currentNote.data.updatedAt);
              const localUpdatedAt = new Date(action.data.updatedAt);

              if (serverUpdatedAt > localUpdatedAt) {
                // Conflict detected: server has newer changes
                console.warn(`⚠️ Conflict detected for note ${realId}`);

                // Option 1: Ask user to resolve (UI prompt)
                toast.error(`Conflict detected for note "${currentNote.data.title}". Please resolve manually.`);

                // Option 2: Merge changes automatically (basic example)
                // Here we just keep server version and mark local as offline again
                const cachedNotes = await getCachedNotes();
                const updatedCache = cachedNotes.map(note => {
                  if (note._id === realId) {
                    return { ...currentNote.data, isOffline: false };
                  }
                  return note;
                });
                await cacheNotes(updatedCache);

                // Skip removing the action so user can retry/resolve later
                break;
              }
              ...
              console.error(`❌ Failed to update note ${realId}:`, error);
